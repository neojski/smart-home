...
export default class {
  ...
  handleDisconnected(device: Device, error?: Error) {
    this.debug('disconnected; cleaning up and reconnecting', { reconnectTimeout, error });
  ...
  async connect() {
    ...
    let device = new Device({ id: this.id, key: this.key, ip: this.ip, heartbeatInterval: 5000 });
    ...
    device.on('disconnected', () => { this.handleDisconnected(device); });
    device.on('error', (e) => { this.handleDisconnected(device, e); });
